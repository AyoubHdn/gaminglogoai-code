...
export default function FreeCreditUnlock() {
    ...
    const unlock = async () => {
        ...
        try {
            ...
            try {
                data = await res.json();
                if (res.status === 403 && (data === null || data === void 0 ? void 0 : data.error)) {
                    setError(data.error);
                    return;
                }
            }
            ...
            if (!res.ok && (data === null || data === void 0 ? void 0 : data.retryAfterMinutes)) {
                setRetryAfter(data.retryAfterMinutes);
                setError(`You already started a survey. Try again in ${data.retryAfterMinutes} minute${data.retryAfterMinutes > 1 ? "s" : ""}.`);
                return;
            }
            if (!res.ok) {
    ...
    return (<>
      ...
      <div className="max-w-lg mx-auto mt-20 text-center">
        <div className="border rounded-lg p-6">
          ...
          <button className="mt-4 px-6 py-3 bg-purple-600 text-white rounded disabled:opacity-60" onClick={unlock} disabled={unlocking}>
            {unlocking ? "Redirecting…" : "Start Survey"}
          </button>

          {/* 🔐 Login hint */}
          {error === "You must sign in to start a survey." && (<p className="mt-3 text-sm text-gray-600">
              Please{" "}
          ...
          {error && error !== "You must sign in to start a survey." && (<p className="mt-3 text-red-500 text-sm">{error}</p>)}

          {/* ⏱ Retry hint */}
          {retryAfter !== null && (<p className="mt-2 text-xs text-gray-500">
