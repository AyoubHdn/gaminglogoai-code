...
import { AiOutlineLeft, AiOutlineRight, AiOutlineCloudUpload } from "react-icons/ai";
...
const typedFaceStylesData = faceStylesData;
const FaceLogoGeneratorPage = () => {
    var _a, _b;
    ...
    const [activeStyleSubTab, setActiveStyleSubTab] = useState("");
    const [selectedStyleBasePrompt, setSelectedStyleBasePrompt] = useState("");
    const [selectedStyleImagePreview, setSelectedStyleImagePreview] = useState(null);
    ...
    const styleCategoryScrollRef = useRef(null);
    const styleSubCategoryScrollRef = useRef(null);
    const [showLeftStyleCategoryArrow, setShowLeftStyleCategoryArrow] = useState(false);
    const [showRightStyleCategoryArrow, setShowRightStyleCategoryArrow] = useState(false);
    const [showLeftStyleSubCategoryArrow, setShowLeftStyleSubCategoryArrow] = useState(false);
    const [showRightStyleSubCategoryArrow, setShowRightStyleSubCategoryArrow] = useState(false);
    const modelOptions = [
    ...
    const handleFaceStyleSelect = useCallback((basePrompt, stylePreviewSrc) => {
    ...
    useEffect(() => {
        ...
        if (hash) {
            for (const mainCategory in typedFaceStylesData) {
                ...
                for (const subCategory in subcategories) {
                    ...
                    const foundItem = items === null || items === void 0 ? void 0 : items.find(item => item.name === hash);
                    if (foundItem) {
        ...
        if (!hashFoundAndSet) {
            ...
            if (firstCategory) {
                ...
                const firstSubCategory = (_a = Object.keys(typedFaceStylesData[firstCategory])) === null || _a === void 0 ? void 0 : _a[0];
                if (firstSubCategory) {
    ...
    useEffect(() => {
        ...
        if (!activeStyleTab || !activeStyleSubTab)
            return;
        const styles = (_a = typedFaceStylesData[activeStyleTab]) === null || _a === void 0 ? void 0 : _a[activeStyleSubTab];
    ...
    const scrollStyleCategories = (direction) => { var _a; return (_a = styleCategoryScrollRef.current) === null || _a === void 0 ? void 0 : _a.scrollBy({ left: direction === "left" ? -200 : 200, behavior: "smooth" }); };
    const scrollStyleSubCategories = (direction) => { var _a; return (_a = styleSubCategoryScrollRef.current) === null || _a === void 0 ? void 0 : _a.scrollBy({ left: direction === "left" ? -200 : 200, behavior: "smooth" }); };
    const { mutate: triggerGenerateFaceLogo, isLoading: isGenerating } = api.faceLogo.generateFaceLogo.useMutation({
        onSuccess(data) {
            ...
            setImagesUrl(data);
            setError("");
            (_a = document.getElementById("results-section")) === null || _a === void 0 ? void 0 : _a.scrollIntoView({ behavior: "smooth", block: "start" });
        },
        onError(error) {
            var _a, _b;
            console.error("CLIENT: onError called. Full Error:", error); // LOG B
            console.error("CLIENT: onError - error.message:", error.message);
            console.error("CLIENT: onError - error.data?.code:", (_a = error.data) === null || _a === void 0 ? void 0 : _a.code);
            if (((_b = error.data) === null || _b === void 0 ? void 0 : _b.code) === 'BAD_REQUEST' && error.message.toLowerCase().includes("enough gaming credits")) {
    ...
    const handleFormSubmit = async (e) => {
        ...
        if (!inputText.trim()) {
            ...
            (_a = document.getElementById("logo-text-input")) === null || _a === void 0 ? void 0 : _a.focus();
        ...
        if (!selectedStyleBasePrompt) {
            setError("Please select an artistic style for your face logo.");
            return;
        }
        ...
        reader.onloadend = () => {
            ...
            if (selectedModel === "flux-kontext-max")
                finalPrompt += `, ultra detailed face, cinematic lighting, sharp focus on face, masterpiece`;
            else
                finalPrompt += `, detailed face, good lighting, professional logo`;
        ...
        try {
            ...
            if (context) {
                ...
                if (pngBlob) {
                    ...
                    alert("Sorry, could not prepare download.");
                }
            }
        }
        catch (dlError) {
    ...
    <input id="face-image-upload" name="face-image-upload" type="file" className="sr-only" ref={fileInputRef} onChange={(event) => {
            void handleImageUpload(event); // handleImageUpload is your async function
        }} accept="image/png, image/jpeg, image/webp"/>                    </span>
          ...
          <section>
            ...
            if (textError)
                setTextError(null); // Clear error on change
        }} placeholder="e.g., Your Gamer Tag (Required)" className={clsx("w-full p-3 text-base border-2 rounded-lg bg-white dark:bg-slate-800 focus:ring-2 focus:border-transparent shadow-sm", 
        // --- CONDITIONAL STYLING ---
        textError
            ? "border-red-500 dark:border-red-500 focus:ring-red-500"
            : "border-slate-300 dark:border-slate-700 focus:ring-purple-500 dark:focus:ring-cyan-500")} aria-label="Text to include in the logo" aria-required="true"/>
          ...
          <section>
            ...
            {activeStyleTab && typedFaceStylesData[activeStyleTab] && Object.keys(typedFaceStylesData[activeStyleTab]).length > 0 && (<div className="relative mb-6 flex items-center">
            ...
            <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 sm:gap-4">
              {((_b = (_a = typedFaceStylesData[activeStyleTab]) === null || _a === void 0 ? void 0 : _a[activeStyleSubTab]) !== null && _b !== void 0 ? _b : []).map((item, idx) => {
            if (!item || !item.src || !item.basePrompt || !item.name) {
                return <div key={`error-style-${idx}`} className="aspect-square bg-red-100 dark:bg-red-900 flex items-center justify-center text-red-700 dark:text-red-300 rounded-lg text-xs p-2">Data Error</div>;
            }
            ...
            <div key={`${activeStyleTab}-${activeStyleSubTab}-${item.src}-${idx}`} id={elementId} // CHANGED: Added ID for jump-links
             className={clsx(`flex flex-col rounded-xl shadow-md hover:shadow-xl dark:bg-slate-700/50 transition-all duration-300 cursor-pointer overflow-hidden border-2`, // Removed aspect-square from here
                selectedStyleImagePreview === item.src ? "border-purple-500 dark:border-cyan-500 scale-105" : "border-transparent hover:border-purple-300 dark:hover:border-cyan-300")} onClick={() => handleFaceStyleSelect(item.basePrompt, item.src)} title={`Select style: ${item.name}`}> 
                    
                    {/* NEW: Wrapper for the image to maintain aspect ratio */}
                    <div className="relative w-full aspect-square group">
                        <Image src={displayImagePath} alt={`Art style preview: ${item.name}`} fill style={{ objectFit: "cover" }} className="transition-transform duration-300 group-hover:scale-110" sizes="(max-width: 640px) 50vw, (max-width: 768px) 33vw, (max-width: 1024px) 25vw, 20vw" loading="lazy" onError={(e) => { e.target.src = "/images/placeholder-style.png"; }}/>
                    </div>
          ...
          <section>
            ...
            <FormGroup className="mb-0">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {modelOptions.map((model) => (<button key={model.value} type="button" onClick={() => setSelectedModel(model.value)} className={clsx(`flex flex-col items-start justify-between border-2 rounded-xl p-4 transition-all duration-200 min-h-[100px] sm:min-h-[120px] h-full text-left text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-800/70 group shadow-sm hover:shadow-lg`, selectedModel === model.value ? "border-purple-500 dark:border-cyan-500 ring-2 ring-purple-500 dark:ring-cyan-400 scale-105 shadow-xl" : "border-slate-300 dark:border-slate-700 hover:border-purple-400 dark:hover:border-cyan-400")}>
        ...
        {imagesUrl.length > 0 && (<section id="results-section" className="mt-12">
            ...
            <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {imagesUrl.map(({ imageUrl }, index) => (<div key={imageUrl} className="relative group rounded-xl shadow-lg hover:shadow-2xl dark:bg-slate-800/70 transition-all duration-300 aspect-square overflow-hidden border-2 border-transparent hover:border-purple-500 dark:hover:border-cyan-500">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-end p-3 space-y-2">
                    <div className="flex gap-2">
                        ...
                        <button type="button" onClick={() => handleOpenSharePopup(imageUrl, "this awesome logo")} // Use icon.imageUrl and icon.prompt
             className="p-2 sm:p-2.5 rounded-full bg-slate-100/80 dark:bg-slate-700/80 hover:bg-white dark:hover:bg-slate-600 text-slate-700 dark:text-slate-100 shadow-md transition-colors disabled:opacity-50" title="Share Logo" aria-label="Share Logo">
        ...
        <SharePopup imageUrl={showSharePopupFor} imageAlt={`Shareable gaming logo ${currentPromptForShare ? 'for ' + currentPromptForShare : ''}`} defaultText={`Check out this logo I made ${currentPromptForShare ? `for "${currentPromptForShare.substring(0, 50)}..."` : ''} with GamingLogoAI!`} siteUrl="https://gaminglogoai.com" // ** REPLACE **
         
        // For collection, the original generator might not be easily known,
        // so link to the main gaming logo maker or homepage.
        generatorUrl={"/gaming-logo-maker"} onClose={() => setShowSharePopupFor(null)}/>)}
      </main>
    </>);
};
