...
import { FaGamepad, FaPalette, FaShapes, FaUserCircle, FaChevronRight } from "react-icons/fa";
...
// --- PAGE COMPONENT ---
const LogoStylesHubPage = ({ styleCategories }) => {
    return (<>
      <Head>
        ...
        <section className="py-12 md:py-16">
          <div className="container mx-auto px-4 sm:px-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {styleCategories.map((category) => (<div key={category.name} className="flex flex-col p-6 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 transition-shadow duration-300 hover:shadow-2xl">
                  ...
                  {category.subItemsPreview.length > 0 && (<div className="mb-4 border-t border-slate-200 dark:border-slate-700 pt-4 space-y-1.5">
                      <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5">Popular examples:</p>
                      {category.subItemsPreview.slice(0, 3).map((item) => (<Link key={item.name} href={item.targetUrl} legacyBehavior={false}>
                  ...
                  <div className="mt-auto pt-4 border-t border-slate-200 dark:border-slate-700 space-y-2">
                    {category.relevantTools.map((tool) => (<Button key={tool.name} onClick={() => window.location.href = tool.url} variant={tool.variant || "primary"} size="sm" fullWidth className="text-xs sm:text-sm">
...
export const getStaticProps = async () => {
    ...
    for (const config of logoCategoriesConfig) {
        const dataSet = gamerStylesData[config.parentKey];
        if (dataSet) {
            ...
            styleCategories.push(Object.assign(Object.assign({}, config), { subItemsPreview, relevantTools: [{ name: "Open Gaming Logo Maker", url: "/gaming-logo-maker", iconName: "pen", variant: "primary" }] }));
        }
    }
    ...
    for (const config of pfpCategoriesConfig) {
        const dataSet = faceStylesData[config.parentKey];
        if (dataSet) {
            ...
            styleCategories.push(Object.assign(Object.assign({}, config), { subItemsPreview, relevantTools: [{ name: "Open AI PFP Maker", url: "/pfp-maker", iconName: "user", variant: "accent" }] }));
        }
    }
    return {
        props: {
