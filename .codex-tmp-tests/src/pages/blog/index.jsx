...
import { getSortedPostsData } from "~/lib/posts"; // Adjust path if your lib/posts.ts is elsewhere
...
const BlogIndexPage = ({ allPostsData }) => {
    ...
    return (<>
      <Head>
        ...
        <meta name="description" content={`Explore articles, tutorials, and insights on creating impactful AI gaming logos, PFP avatars, and effective gaming branding with ${siteName}.`}/>
      ...
      <main className="container mx-auto max-w-3xl px-4 sm:px-6 py-12 sm:py-16 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">
        ...
        <section>
          {allPostsData && allPostsData.length > 0 ? (<ul className="space-y-8 sm:space-y-10">
              {allPostsData.map(({ slug, date, title, excerpt, tags, coverImage, author }) => {
                ...
                return (<li key={slug} className="p-6 sm:p-8 rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                    <article className="flex flex-col sm:flex-row gap-6">
                      {coverImage && (<div className="flex-shrink-0 w-full sm:w-1/3 md:w-1/4 aspect-video sm:aspect-square relative rounded-lg overflow-hidden">
                          <Link href={`/blog/${slug}`} legacyBehavior={false} className="block w-full h-full">
                              <Image src={coverImage} alt={`${displayTitle} cover image`} layout="fill" objectFit="cover" className="transition-transform duration-300 hover:scale-105" unoptimized={true}/>
                          </Link>
                        </div>)}
                      <div className="flex-1">
                        ...
                        {tags && tags.length > 0 && (<div className="flex flex-wrap gap-2 mb-4">
                            {tags.map(tag => (<span key={tag} className="text-xs bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 px-2.5 py-1 rounded-full">
                                #{tag}
                              </span>))}
                          </div>)}
            ...
            </ul>) : (<div className="text-center py-10">
