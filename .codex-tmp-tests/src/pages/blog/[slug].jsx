...
const mdxComponents = {
    ...
    h2: (props) => (<h2 className="text-2xl sm:text-3xl font-semibold mt-10 mb-5 text-slate-800 dark:text-white !leading-snug" {...props}/>),
    h3: (props) => (<h3 className="text-xl sm:text-2xl font-semibold mt-8 mb-4 text-slate-800 dark:text-white !leading-snug" {...props}/>),
    p: (props) => (<p className="text-base sm:text-lg leading-relaxed my-5 sm:my-6 text-slate-700 dark:text-slate-300" {...props}/>),
    ul: (props) => (<ul className="list-disc list-inside my-5 sm:my-6 pl-5 space-y-2 text-slate-700 dark:text-slate-300" {...props}/>),
    ol: (props) => (<ol className="list-decimal list-inside my-5 sm:my-6 pl-5 space-y-2 text-slate-700 dark:text-slate-300" {...props}/>),
    li: (props) => (<li className="text-base sm:text-lg leading-relaxed mb-1.5" {...props}/>),
    a: (props) => {
    ...
    img: (props) => {
        if (!props.src)
            return null;
        return (<span className="block my-8 relative aspect-video shadow-lg rounded-lg overflow-hidden">
        <Image src={typeof props.src === "string" ? props.src : ""} alt={props.alt || "GamingLogoAI blog post illustration"} fill style={{ objectFit: "contain" }} // Use "contain" for blog images to see whole image
         sizes="(max-width: 768px) 100vw, 700px" className="rounded-lg" unoptimized={true}/>
      </span>);
    },
    blockquote: (props) => (<blockquote className="my-6 pl-4 italic border-l-4 border-purple-500 dark:border-cyan-500 text-slate-600 dark:text-slate-400" {...props}/>),
    ...
    pre: (props) => ( // For code blocks ```code```
...
const BlogPostPage = ({ postData }) => {
    ...
    let validDateForDateTimeAttr = undefined;
    if (postData.date && typeof postData.date === 'string') {
        try {
            ...
            if (isValid(parsedDate)) {
                ...
                console.warn(`BlogPostPage [${postData.slug}]: Invalid date string: "${postData.date}"`);
            }
        }
        catch (e) {
            console.error(`BlogPostPage [${postData.slug}]: Error parsing date "${postData.date}":`, e);
        }
    }
    else {
        console.warn(`BlogPostPage [${postData.slug}]: Date missing/invalid.`);
    }
    ...
    return (<>
      <Head>
        ...
        {postData.coverImage && (<meta property="og:image" content={`${siteUrl}${postData.coverImage.startsWith('/') ? '' : '/'}${postData.coverImage}`}/>)}
        ...
        {postData.tags && postData.tags.map(tag => (<meta property="article:tag" content={tag} key={`og-tag-${tag}`}/>))}
        ...
        {postData.coverImage && (<meta name="twitter:image" content={`${siteUrl}${postData.coverImage.startsWith('/') ? '' : '/'}${postData.coverImage}`}/>)}
      ...
      <main className="container mx-auto max-w-3xl px-4 sm:px-6 py-12 sm:py-16 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">
        ...
        <article className="text-slate-700 dark:text-slate-300"> {/* Basic text color */}
          <header className="mb-8 border-b border-slate-200 dark:border-slate-700 pb-6">
            ...
            {postData.coverImage && (<div className="mt-8 mb-4 relative aspect-[16/9] rounded-lg overflow-hidden shadow-lg">
                <Image src={postData.coverImage} alt={`${displayTitle} cover image`} fill style={{ objectFit: "cover" }} priority sizes="(max-width: 768px) 100vw, 800px" unoptimized={true}/>
              </div>)}
          </header>
          
          {postData.content ? (<MDXRemote {...postData.content} components={mdxComponents}/>) : (<p className="text-center text-slate-500 dark:text-slate-400 py-10">Content for this post is currently unavailable.</p>)}

          {/* ... (Tags and Back to Blog link as before, maybe with 'not-prose' if still using base prose on article) ... */}
          {/* Tags Section */}
          {postData.tags && postData.tags.length > 0 && (<div className="mt-10 pt-8 border-t border-slate-200 dark:border-slate-700">
...
export const getStaticProps = async ({ params }) => {
    if (!(params === null || params === void 0 ? void 0 : params.slug) || typeof params.slug !== 'string') {
