...
const CommunityPage = () => {
    ...
    return (<>
      <Head>
        ...
        <meta name="description" content="Explore a gallery of AI-generated gaming logos created by the Gaming Logo AI community. Get inspired for your next team emblem, streamer avatar, or game icon."/>
      ...
      <main className="flex min-h-screen mt-16 sm:mt-24 flex-col container mx-auto gap-8 px-4 sm:px-8 py-8 bg-gray-100 dark:bg-slate-900 text-slate-800 dark:text-slate-200">
        ...
        {iconsQuery.isLoading && (<div className="flex justify-center items-center py-10">
        ...
        {iconsQuery.data && iconsQuery.data.length === 0 && (<div className="text-center py-10 bg-white dark:bg-slate-800 rounded-lg shadow-md">
        ...
        {iconsQuery.data && iconsQuery.data.length > 0 && (<ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {iconsQuery.data.map((icon) => {
                var _a, _b, _c;
                return (<li key={getS3ImageUrl(icon)} className="relative group bg-white dark:bg-slate-800 rounded-lg shadow-md overflow-hidden aspect-square cursor-pointer" onClick={() => openPopup(icon)} title={`View logo: ${icon.prompt || 'Community Gaming Logo'}`}>
                <Image className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" width={512} height={512} alt={(_a = icon.prompt) !== null && _a !== void 0 ? _a : `Community gaming logo ${getS3ImageUrl(icon)}`} src={getS3ImageUrl(icon)} priority={false}/>
                 <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <p className="text-white text-xs truncate" title={(_b = icon.prompt) !== null && _b !== void 0 ? _b : ''}>
                        {(_c = icon.prompt) !== null && _c !== void 0 ? _c : 'Awesome Gaming Logo'}
                    </p>
                </div>
                </li>);
            })}
            </ul>)}
        {popupImage && (<div className="fixed inset-0 z-50 bg-black bg-opacity-80 flex items-center justify-center p-4" onClick={closePopup}>
            <div className="relative bg-white dark:bg-slate-800 p-1 rounded-lg shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <button onClick={closePopup} className="absolute -top-3 -right-3 bg-purple-600 dark:bg-cyan-500 text-white rounded-full p-1.5 hover:opacity-80 focus:outline-none z-10 shadow-md" title="Close Fullscreen" aria-label="Close Fullscreen">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"/>
                </svg>
              </button>
              <Image src={popupImage} alt="Fullscreen community gaming logo" width={800} height={800} style={{ objectFit: 'contain', maxWidth: '90vw', maxHeight: '90vh' }} className="rounded"/>
            </div>
          </div>)}
      </main>
    </>);
