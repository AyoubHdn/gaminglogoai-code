...
const GameLogoPage = () => {
    ...
    const [selectedStyleImageSrc, setSelectedStyleImageSrc] = useState(null);
    const [popupImage, setPopupImage] = useState(null);
    const [activeTab, setActiveTab] = useState("");
    const [activeSubTab, setActiveSubTab] = useState("");
    const [showSharePopupFor, setShowSharePopupFor] = useState(null);
    const [currentPromptForShare, setCurrentPromptForShare] = useState("");
    ...
    const typedGamerStylesData = gamerStylesData; // Cast to our defined type
    const handleStyleSelect = useCallback((basePrompt, src) => {
        setSelectedStyleImageSrc(src);
        setForm((prev) => (Object.assign(Object.assign({}, prev), { basePrompt })));
        setError("");
    ...
    useEffect(() => {
            ...
            for (const mainCategory in typedGamerStylesData) {
                ...
                if (subcategories && Object.prototype.hasOwnProperty.call(subcategories, hash)) {
                    ...
                    setTimeout(() => {
                        ...
                        element === null || element === void 0 ? void 0 : element.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'center' });
                    }, 150);
                    break;
                }
            }
        ...
        if (!hashFoundAndSet) {
            ...
            if (firstCategory) {
                ...
                const firstSubCategory = (_a = Object.keys(typedGamerStylesData[firstCategory])) === null || _a === void 0 ? void 0 : _a[0];
                if (firstSubCategory) {
    ...
    useEffect(() => {
        ...
        if (!activeTab || !activeSubTab)
            return;
        const styles = (_a = typedGamerStylesData[activeTab]) === null || _a === void 0 ? void 0 : _a[activeSubTab];
    ...
    const scrollCategories = (direction) => { var _a; return (_a = categoryScrollRef.current) === null || _a === void 0 ? void 0 : _a.scrollBy({ left: direction === "left" ? -200 : 200, behavior: "smooth" }); };
    const handleSubCategoryScroll = () => handleScroll(subcategoryScrollRef, setShowLeftSubCategoryArrow, setShowRightSubCategoryArrow);
    const scrollSubCategories = (direction) => { var _a; return (_a = subcategoryScrollRef.current) === null || _a === void 0 ? void 0 : _a.scrollBy({ left: direction === "left" ? -200 : 200, behavior: "smooth" }); };
    const { mutate: triggerGenerateIcon, isLoading: isGenerating } = api.generate.generateIcon.useMutation({
        onSuccess(data) { var _a; setImagesUrl(data); setError(""); (_a = document.getElementById("results-section")) === null || _a === void 0 ? void 0 : _a.scrollIntoView({ behavior: "smooth", block: "start" }); },
        onError(error) {
            var _a;
            console.error("Generation Error (Full TRPCClientError object):", error);
            if (((_a = error.data) === null || _a === void 0 ? void 0 : _a.code) === 'BAD_REQUEST' && error.message.toLowerCase().includes("enough gaming credits")) {
        ...
        try {
            ...
            if (context) {
                ...
                if (pngBlob) {
                    ...
                    link.href = blobUrl;
                    const safeName = (form.name.trim() || (promptText === null || promptText === void 0 ? void 0 : promptText.replace(/[^a-z0-9_]+/gi, '_').substring(0, 30)) || "gaming-logo").replace(/[^a-z0-9_]+/gi, '_');
            ...
            console.error("Error downloading the image:", dlError);
            alert("Could not download image. Please try again.");
        }
    };
    const openPopup = (imageUrl) => setPopupImage(imageUrl);
    const closePopup = () => setPopupImage(null);
    const handleOpenSharePopup = (imageUrl, promptOrName) => {
    ...
    return (<>
      ...
      <main className="container mx-auto max-w-screen-lg mb-24 flex flex-col px-4 sm:px-8 py-8 text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-900">
        ...
        <form className="flex flex-col gap-10" onSubmit={handleFormSubmit}>
          <section>
            ...
            <FormGroup className="mb-0">
              <Input required value={form.name} onChange={(e) => setForm((prev) => (Object.assign(Object.assign({}, prev), { name: e.target.value })))} placeholder="e.g., ShadowBlade, Pixel Prowlers, YourStreamName" className="w-full p-3 text-base border-2 border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 focus:ring-2 focus:ring-purple-500 dark:focus:ring-cyan-500 focus:border-transparent shadow-sm" aria-label="Gamer Tag or Team Name"/>
            </FormGroup>
          </section>
          ...
          <section>
            ...
            {activeTab && typedGamerStylesData[activeTab] && Object.keys(typedGamerStylesData[activeTab]).length > 0 && (<div className="relative mb-6 flex items-center">
            ...
            <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 sm:gap-4">
              {((_b = (_a = typedGamerStylesData[activeTab]) === null || _a === void 0 ? void 0 : _a[activeSubTab]) !== null && _b !== void 0 ? _b : []).map((item, idx) => {
            ...
            return (<div key={`${activeTab}-${activeSubTab}-${item.src}-${idx}`} className={clsx(`relative rounded-xl shadow-md hover:shadow-xl dark:bg-slate-700/50 transition-all duration-300 cursor-pointer aspect-square overflow-hidden group border-2`, selectedStyleImageSrc === item.src ? "border-purple-500 dark:border-cyan-500 scale-105" : "border-transparent hover:border-purple-300 dark:hover:border-cyan-300")} onClick={() => handleStyleSelect(item.basePrompt, item.src)} title={`Select style: ${item.basePrompt.substring(0, 50)}...`}>
                    <img src={displayImagePath} alt={`Style preview for: ${item.basePrompt.substring(0, 40)}...`} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" loading="lazy" onError={(e) => {
                    ...
                    <button type="button" onClick={(e) => { e.stopPropagation(); openPopup(displayImagePath); }} className="absolute top-1.5 right-1.5 z-10 p-1.5 rounded-full bg-black/30 hover:bg-black/60 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-200 focus:outline-none" title="View Fullscreen" aria-label="View Fullscreen">
          ...
          <section>
                ...
                {[
            {
                ...
                getPreviewImage: (baseSrc) => baseSrc, // Shows selected style directly
            },
            {
                ...
                getPreviewImage: (baseSrc) => baseSrc ? baseSrc.replace(/(\.[^.]+)$/, "e$1") : null,
            },
        ].map((model) => {
            const previewImageSrc = model.getPreviewImage(selectedStyleImageSrc) || "/images/placeholder-logo.png";
            return (<button key={model.value} type="button" onClick={() => setSelectedModel(model.value)} className={clsx(`flex flex-col items-stretch justify-between border-2 rounded-xl p-4 transition-all duration-200 h-full text-left text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-800/70 group shadow-sm hover:shadow-lg`, selectedModel === model.value ? "border-purple-500 dark:border-cyan-500 ring-2 ring-purple-500 dark:ring-cyan-400 scale-105 shadow-xl" : "border-slate-300 dark:border-slate-700 hover:border-purple-400 dark:hover:border-cyan-400")}>
                      <div className="relative w-full aspect-square mb-3 overflow-hidden rounded-lg bg-slate-200 dark:bg-slate-700 border border-slate-300 dark:border-slate-600">
                        <Image src={previewImageSrc} alt={`${model.name} preview`} layout="fill" objectFit="cover" className="transition-transform duration-300 group-hover:scale-105" onError={(e) => { e.target.src = "/images/placeholder-logo.png"; }} unoptimized={true}/>
          ...
          <section>
            ...
            <FormGroup className="mb-0">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {aspectRatios.map((ratio) => (<button key={ratio.value} type="button" onClick={() => setSelectedAspectRatio(ratio.value)} className={clsx(`relative flex flex-col items-center justify-start border-2 rounded-xl p-3 pt-4 transition-all duration-200 group text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-800/70 h-32 sm:h-36 shadow-sm hover:shadow-lg`, selectedAspectRatio === ratio.value ? "border-purple-500 dark:border-cyan-500 ring-2 ring-purple-500 dark:ring-cyan-400 scale-105 shadow-xl" : "border-slate-300 dark:border-slate-700 hover:border-purple-400 dark:hover:border-cyan-400")} title={`Select aspect ratio: ${ratio.label} (${ratio.description})`}>
                      <div className="w-full h-[50px] sm:h-[60px] flex items-center justify-center mb-1 sm:mb-2">
                        <div className={clsx("bg-slate-300 dark:bg-slate-600 rounded-md shadow-inner group-hover:bg-slate-400 dark:group-hover:bg-slate-500 transition-colors border border-slate-400 dark:border-slate-500", ratio.tailwindClass)} style={ratio.value === "16:9" ? { width: '80%', height: 'auto' } : { height: '80%', width: 'auto' }}></div>
                      </div>
          ...
          <section>
            ...
            <FormGroup className="mb-0">
              ...
              <Input required id="numberofImages" type="number" min={1} max={4} value={form.numberofImages} onChange={(e) => {
            const num = parseInt(e.target.value, 10);
            const maxVal = 4;
            if (num > maxVal) {
                setForm((prev) => (Object.assign(Object.assign({}, prev), { numberofImages: maxVal.toString() })));
            }
            else if (num < 1 && e.target.value !== "") {
                setForm((prev) => (Object.assign(Object.assign({}, prev), { numberofImages: "1" })));
            }
            else {
                setForm((prev) => (Object.assign(Object.assign({}, prev), { numberofImages: e.target.value })));
            }
        }} placeholder={"1-4"} className="w-full p-3 text-base border-2 border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 focus:ring-2 focus:ring-purple-500 dark:focus:ring-cyan-500 focus:border-transparent shadow-sm"/>
            </FormGroup>
          </section>

          {/* Error Display */}
          {error && (<div className={`my-6 p-4 border-l-4 rounded-md shadow-md ${error === "INSUFFICIENT_CREDITS" ? "bg-yellow-50 border-yellow-500 dark:bg-yellow-900/30 dark:border-yellow-600" : "bg-red-50 border-red-500 dark:bg-red-900/30 dark:border-red-600"}`}>
              <div className="flex">
                <div className="flex-shrink-0">
                  {error === "INSUFFICIENT_CREDITS" ? (<svg className="h-5 w-5 text-yellow-500 dark:text-yellow-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.332-.211 3.001-1.742 3.001H4.42c-1.531 0-2.493-1.669-1.743-3.001l5.58-9.92zM10 10.5a1 1 0 011-1h.01a1 1 0 110 2H11a1 1 0 01-1-1zm.01-4.502a1 1 0 00-.998 1.116l.244 2.684a1 1 0 001.99-.182l-.244-2.684a1 1 0 00-.992-.934z" clipRule="evenodd"/></svg>) : (<svg className="h-5 w-5 text-red-500 dark:text-red-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm-.707-4.293a1 1 0 001.414 0L12 12.414l1.293 1.293a1 1 0 001.414-1.414L13.414 11l1.293-1.293a1 1 0 00-1.414-1.414L12 9.586 10.707 8.293a1 1 0 00-1.414 1.414L10.586 11l-1.293 1.293a1 1 0 000 1.414z" clipRule="evenodd"/></svg>)}
                </div>
                <div className="ml-3">
        ...
        {imagesUrl.length > 0 && (<section id="results-section" className="mt-12">
            ...
            <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {imagesUrl.map(({ imageUrl }, index) => (<div key={index} className="relative group rounded-xl shadow-lg hover:shadow-2xl dark:bg-slate-800/70 transition-all duration-300 aspect-square overflow-hidden border-2 border-transparent hover:border-purple-500 dark:hover:border-cyan-500">
            ...
            className="p-2 sm:p-2.5 rounded-full bg-slate-100/80 dark:bg-slate-700/80 hover:bg-white dark:hover:bg-slate-600 text-slate-700 dark:text-slate-100 shadow-md transition-colors" title="Share Logo" aria-label="Share Logo">
        ...
        {popupImage && (<div className="fixed inset-0 z-[100] bg-black bg-opacity-80 flex items-center justify-center p-4 backdrop-blur-sm" onClick={closePopup}>
            <div className="relative bg-slate-100 dark:bg-slate-800 p-2 rounded-lg shadow-2xl max-w-3xl max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
              ...
              <Image src={popupImage} alt="Fullscreen generated gaming logo" width={1024} height={1024} style={{ objectFit: 'contain', width: 'auto', height: 'auto', maxHeight: '85vh', maxWidth: 'calc(100vw - 4rem)' }} className="rounded-md" unoptimized={true}/>
            </div>
          </div>)}
        {showSharePopupFor && (<SharePopup imageUrl={showSharePopupFor} imageAlt={`Shareable gaming logo ${currentPromptForShare ? 'for ' + currentPromptForShare : ''}`} defaultText={`Check out this logo I made ${currentPromptForShare ? `for "${currentPromptForShare.substring(0, 50)}..."` : ''} with GamingLogoAI!`} siteUrl="https://gaminglogoai.com" // Replace with your actual site URL
         generatorUrl={router.pathname.includes('face-logo-generator') ? "/face-logo-generator" : "/gaming-logo-maker"
