...
const EmoteGeneratorPage = () => {
    ...
    const emoteScrollRef = useRef(null);
    const scrollEmotes = (dir) => {
...
${selectedStyle === null || selectedStyle === void 0 ? void 0 : selectedStyle.basePrompt}.
Neutral expression.
Centered face.
...
`;
    ...
    const getEmoteExampleSrc = (exampleFile) => {
        return useText
    ...
    return (<>
      ...
      <main className="container mx-auto max-w-4xl px-4 py-12">
        ...
        <section className="mb-8">
          ...
          <label className="flex flex-col items-center justify-center border-2 border-dashed rounded-xl p-6 cursor-pointer hover:border-purple-400 transition">
            {uploadedPreview ? (<Image src={uploadedPreview} alt="Preview" width={160} height={160} className="rounded-lg object-cover"/>) : (<span className="text-slate-500 text-sm">
                Click to upload (PNG, JPG)
              </span>)}
            <input type="file" className="hidden" accept="image/png,image/jpeg" onChange={(e) => { var _a; return handleUpload((_a = e.target.files) === null || _a === void 0 ? void 0 : _a[0]); }}/>
          </label>
        </section>
       ...
       {/* <section className="mb-8">
        ...
        <div className="relative mb-4 flex items-center">
            {showLeftCat && (<button type="button" onClick={() => {
                var _a;
                return (_a = categoryScrollRef.current) === null || _a === void 0 ? void 0 : _a.scrollBy({
                    left: -200,
                    behavior: "smooth",
                });
            ...
            <div ref={categoryScrollRef} onScroll={() => handleScroll(categoryScrollRef, setShowLeftCat, setShowRightCat)} className="flex gap-2 overflow-x-auto no-scrollbar px-6">
            {Object.keys(emoteBaseStyles).map((cat) => (<button key={cat} type="button" onClick={() => {
                var _a;
                ...
                if (!firstSub)
                    return;
                const firstStyle = (_a = categoryStyles[firstSub]) === null || _a === void 0 ? void 0 : _a[0];
                if (!firstStyle)
                    return;
                setActiveCategory(cat);
                setActiveSubCategory(firstSub);
                setSelectedStyle(firstStyle);
            }} className={clsx("px-4 py-2 rounded-lg font-semibold whitespace-nowrap", activeCategory === cat
            ...
            {showRightCat && (<button type="button" onClick={() => {
                var _a;
                return (_a = categoryScrollRef.current) === null || _a === void 0 ? void 0 : _a.scrollBy({
                    left: 200,
                    behavior: "smooth",
        ...
        {/* ================= SUBCATEGORY TABS ================= */}
        {activeCategory && (<div className="relative mb-6 flex items-center">
            {showLeftSub && (<button type="button" onClick={() => {
                    var _a;
                    return (_a = subCategoryScrollRef.current) === null || _a === void 0 ? void 0 : _a.scrollBy({
                        left: -200,
                        behavior: "smooth",
                    });
            ...
            <div ref={subCategoryScrollRef} onScroll={() => handleScroll(subCategoryScrollRef, setShowLeftSub, setShowRightSub)} className="flex gap-2 overflow-x-auto no-scrollbar px-6">
                {Object.keys((_a = emoteBaseStyles[activeCategory]) !== null && _a !== void 0 ? _a : {}).map((sub) => (<button key={sub} type="button" onClick={() => {
                    var _a, _b;
                    const firstStyle = (_b = (_a = emoteBaseStyles[activeCategory]) === null || _a === void 0 ? void 0 : _a[sub]) === null || _b === void 0 ? void 0 : _b[0];
                    if (!firstStyle)
                        return;
                    setActiveSubCategory(sub);
                    setSelectedStyle(firstStyle);
                }} className={clsx("px-3 py-1.5 rounded-md text-sm whitespace-nowrap", activeSubCategory === sub
            ...
            {showRightSub && (<button type="button" onClick={() => {
                    var _a;
                    return (_a = subCategoryScrollRef.current) === null || _a === void 0 ? void 0 : _a.scrollBy({
                        left: 200,
                        behavior: "smooth",
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {((_c = (_b = emoteBaseStyles[activeCategory]) === null || _b === void 0 ? void 0 : _b[activeSubCategory]) !== null && _c !== void 0 ? _c : []).map((style) => (<button key={style.id} type="button" onClick={() => setSelectedStyle(style)} className={clsx("rounded-xl border-2 overflow-hidden transition", (selectedStyle === null || selectedStyle === void 0 ? void 0 : selectedStyle.id) === style.id
                ? "border-purple-500 scale-105"
                : "border-transparent hover:border-purple-300")}>
                <div className="relative aspect-square">
                    <Image src={style.preview} alt={style.name} fill className="object-cover"/>
                </div>
        ...
        {error && (<div className="mb-6 p-4 bg-red-100 text-red-700 rounded-lg">
            {error}
          </div>)}

        <Button onClick={handleGenerate} isLoading={generateBaseImage.isLoading} className="w-full py-4 text-lg">
          Generate Base Image ({BASE_COST} credits)
        </Button>

        {/* Result */}
        {baseImageUrl && (<section className="mt-12 text-center">
            <h2 className="text-2xl font-semibold mb-4">
            Base Image Ready
            </h2>

            <div className="mx-auto relative w-64 h-64 rounded-xl overflow-hidden shadow-lg">
            <Image src={baseImageUrl} alt="Generated base emote" fill className="object-cover"/>
            </div>
      ...
      {baseImageUrl && (<section className="mt-14 rounded-2xl border dark:bg-slate-900 border-purple-500 bg-slate-50 p-6 shadow-sm ">
            {/* Header */}
            ...
            <div className="inline-flex rounded-xl border bg-slate-100 p-1">
                <button type="button" onClick={() => setUseText(true)} className={clsx("px-4 py-2 rounded-lg text-sm font-medium transition", useText
                ? "bg-white shadow text-slate-900"
                : "text-slate-500 hover:text-slate-700")}>
                With Text
                </button>

                <button type="button" onClick={() => setUseText(false)} className={clsx("px-4 py-2 rounded-lg text-sm font-medium transition", !useText
                ? "bg-white shadow text-slate-900"
                : "text-slate-500 hover:text-slate-700")}>
                No Text
                </button>
            ...
            {useText && (<div className="mb-8">
                ...
                {[
                    ...
                    "#9c27b0",
                    "#ff2fd4",
                ].map((color) => (<button key={color} type="button" onClick={() => setTextColor(color)} className={clsx("h-9 w-9 rounded-full border-2 transition", textColor === color
                        ? "border-slate-900 scale-110"
                        : "border-slate-300 hover:scale-105")} style={{ backgroundColor: color }} aria-label={`Select ${color}`}/>))}
                </div>
            </div>)}
            ...
            <div className="relative -mx-6">
              {/* Left button */}
              <button type="button" onClick={() => scrollEmotes("left")} className="absolute left-2 top-1/2 z-10 -translate-y-1/2 rounded-full dark:bg-slate-700 shadow border p-2 hover:bg-slate-100">
                ←
              </button>

              {/* Right button */}
              <button type="button" onClick={() => scrollEmotes("right")} className="absolute right-2 top-1/2 z-10 -translate-y-1/2 rounded-full dark:bg-slate-700 shadow border p-2 hover:bg-slate-100">
                →
              </button>

              <div ref={emoteScrollRef} className="overflow-x-auto no-scrollbar">
                <div className="flex gap-4 px-6 pb-4 ">
                  {emotes.map((emote) => {
                const active = selectedEmotes.includes(emote.key);
                return (<button key={emote.key} type="button" onClick={() => setSelectedEmotes((prev) => active
                        ? prev.filter((e) => e !== emote.key)
                        : [...prev, emote.key])} className={clsx("min-w-[120px] shrink-0 rounded-xl border-2 transition-all dark:bg-slate-500", active
                        ? "border-purple-500 ring-2 ring-purple-200"
                        : "border-slate-200 hover:border-purple-300")}>
                        <div className="relative aspect-square bg-slate-50 dark:bg-slate-700">
                          <Image src={getEmoteExampleSrc(emote.example)} alt={emote.label} fill className="object-cover"/>
                        </div>
            ...
            <Button isLoading={generateEmotes.isLoading} disabled={selectedEmotes.length === 0} onClick={() => {
                if (!baseImageUrl) {
            ...
            {generatedEmotes.length > 0 && (<section className="mt-16">
                <h2 className="text-2xl font-semibold mb-6 text-center">
                Your Emotes
                </h2>

                <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                {generatedEmotes.map(({ emote, imageUrl }) => (<div key={emote} className="relative rounded-xl overflow-hidden border shadow-sm">
                    <Image src={imageUrl} alt={`${emote} emote`} width={256} height={256} className="object-cover"/>

                    <div className="absolute bottom-2 right-2">
                        <Button size="sm" onClick={() => downloadImage(imageUrl, `${emote}.png`)}>
                        Download
                        </Button>
                    </div>
                    </div>))}
