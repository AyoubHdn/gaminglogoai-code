...
const SuccessPage = () => {
    ...
    useEffect(() => {
        ...
        if (typeof window !== "undefined" && window.gtag) { // gtag is now correctly typed from global.ts
            const transactionId = router.query.session_id || `tx_${Date.now()}`;
            let purchaseValue = 1.0; // Default value
            if (router.query.amount && !isNaN(parseFloat(router.query.amount))) {
                purchaseValue = parseFloat(router.query.amount);
            }
    ...
    return (<>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-slate-900 px-4">
        <div className="bg-white dark:bg-slate-800 shadow-xl rounded-lg p-8 sm:p-12 text-center max-w-md w-full">
          ...
          <button className="w-full font-semibold py-3 px-4 rounded-lg transition-all duration-300 ease-in-out
                       bg-purple-600 hover:bg-purple-700 text-white
                       dark:bg-cyan-500 dark:hover:bg-cyan-600 dark:text-slate-900
                       transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 dark:focus:ring-cyan-400" onClick={() => {
