...
const CollectionPage = () => {
    ...
    const [isDownloading, setIsDownloading] = useState(null); // Track download state per image
    ...
    const getS3ImageUrl = (icon) => {
        var _a;
        return `${s3BucketUrl}/${(_a = icon.imageKey) !== null && _a !== void 0 ? _a : icon.id}`;
    };
    const handleDownload = async (icon, prompt) => {
        ...
        try {
            ...
            const pngBlob = await new Promise((resolve) => canvas.toBlob((blob) => resolve(blob), "image/png", 1.0) // Quality 1.0 for PNG
            );
            if (pngBlob) {
    ...
    const handleOpenSharePopup = (fullImageUrlToShare, promptText) => {
    ...
    if (status === "loading") {
        return (<div className="flex min-h-screen items-center justify-center bg-gray-100 dark:bg-slate-900">
    ...
    if (status === "unauthenticated") {
        return (<div className="flex min-h-screen flex-col items-center justify-center bg-gray-100 px-4 text-center dark:bg-slate-900">
        ...
        <Link href="/api/auth/signin" className="rounded-lg bg-purple-600 px-6 py-3 font-semibold text-white transition-all
    ...
    return (<>
      <Head>
        ...
        <meta name="description" content="Access and manage all your AI-generated gaming logos. View, download, and organize your unique designs created with Gaming Logo AI."/>
      ...
      <main className="container mx-auto mt-16 flex min-h-screen flex-col gap-8 bg-gray-100 px-4 py-8 text-slate-800 sm:mt-24 sm:px-8 dark:bg-slate-900 dark:text-slate-200">
        ...
        {iconsQuery.isError && (<p className="text-center text-lg text-red-500">
            Could not load your logos. Please try again.
          </p>)}

        {iconsQuery.data && iconsQuery.data.length === 0 && (<div className="rounded-lg bg-white py-10 text-center shadow-md dark:bg-slate-800">
            ...
            <Link href="/gaming-logo-maker" className="rounded-lg bg-purple-600 px-6 py-3 font-semibold text-white transition-all
                            duration-300 ease-in-out hover:bg-purple-700
                            dark:bg-cyan-500 dark:text-slate-900 dark:hover:bg-cyan-600">
        ...
        {iconsQuery.data && iconsQuery.data.length > 0 && (<ul className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
            {iconsQuery.data.map((icon) => {
                var _a;
                const fullIconS3Url = getS3ImageUrl(icon);
                return (<li key={icon.id} className="group relative aspect-square overflow-hidden rounded-lg bg-white shadow-md dark:bg-slate-800">
                  <Image className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" width={512} // Keep for aspect ratio hint, actual display controlled by CSS
                 height={512} alt={(_a = icon.prompt) !== null && _a !== void 0 ? _a : `AI generated gaming logo ${icon.id}`} src={fullIconS3Url} priority={false} // Set to true for first few images if you want them to load faster
                />
                  ...
                  <div className="absolute inset-0 flex items-center justify-center gap-2 bg-black bg-opacity-0 transition-opacity duration-300 group-hover:bg-opacity-50">
                    <button onClick={() => openPopup(icon)} className="rounded-full bg-slate-700 p-3 text-white
                               opacity-0 shadow-lg transition-opacity duration-300 hover:bg-purple-600 focus:outline-none group-hover:opacity-100 dark:hover:bg-cyan-500" title="View Fullscreen" aria-label="View Fullscreen">
                    ...
                    <button onClick={() => {
                    ...
                    }} disabled={isDownloading === icon.id} className={`rounded-full bg-slate-700 p-3 text-white
                      ...
                      {isDownloading === icon.id ? (<svg className="h-5 w-5 animate-spin text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        {popupImage && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 p-4">
            <div className="relative rounded-lg bg-white p-2 shadow-2xl dark:bg-slate-800">
              <button onClick={closePopup} className="absolute -right-4 -top-4 z-10 rounded-full bg-purple-600 p-2 text-white shadow-md hover:opacity-80 focus:outline-none dark:bg-cyan-500" title="Close Fullscreen" aria-label="Close Fullscreen">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"/>
                </svg>
              </button>
              <Image // Using Next/Image for optimized loading in popup too
         src={popupImage} alt="Fullscreen gaming logo view" width={1024} // Set a reasonable max width for popup
        ...
        {showSharePopupFor && (<SharePopup imageUrl={showSharePopupFor} imageAlt={`Shareable gaming logo ${currentPromptForShare ? "for " + currentPromptForShare : ""}`} defaultText={`Check out this logo I made ${currentPromptForShare
                ? `for "${currentPromptForShare.substring(0, 50)}..."`
                : ""} with GamingLogoAI!`} siteUrl="https://gaminglogoai.com" // ** REPLACE **
