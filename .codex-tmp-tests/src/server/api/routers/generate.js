...
const generateIcon = async (prompt, numberOfImages = 1, aspectRatio = "1:1", model = "flux-schnell") => {
    let path;
    let input;
    let outputs = [];
...
export const generateRouter = createTRPCRouter({
    generateIcon: protectedProcedure
        ...
        const totalCredits = modelConfig.credits *
        ...
        // Generate images
        const b64Images = await generateIcon(input.prompt, input.numberOfImages, input.aspectRatio, input.model);
        // Store images in DB & upload to S3
        const createdIcons = await Promise.all(b64Images.map(async (image) => {
