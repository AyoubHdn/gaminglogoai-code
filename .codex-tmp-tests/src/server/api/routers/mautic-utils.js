...
export async function updateMauticContact(endpoint, method, payload) {
    var _a, _b;
    ...
    if (!response.ok) {
        ...
        const errorMessage = ((_b = (_a = errorData === null || errorData === void 0 ? void 0 : errorData.errors) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.message) || (errorData === null || errorData === void 0 ? void 0 : errorData.message) || `Mautic API Error: ${response.status}`;
...
async function findMauticContactByEmail(email) {
    console.log(`MAUTIC_UTIL: Searching for contact with email: ${email}`);
    try {
        const searchResult = await makeMauticApiCall(`contacts?search=email:${encodeURIComponent(email)}&limit=1`, 'GET');
...
// Main sync function
export async function syncUserToMautic(contactInput, currentBrand) {
    var _a, _b, _c, _d, _e;
    ...
    const mauticPayloadForApi = {
        email: contactInput.email,
        firstname: ((_a = contactInput.name) === null || _a === void 0 ? void 0 : _a.split(' ')[0]) || undefined,
        lastname: ((_b = contactInput.name) === null || _b === void 0 ? void 0 : _b.split(' ').slice(1).join(' ')) || undefined,
    ...
    try {
        ...
        if (existingMauticContact && typeof existingMauticContact.id === 'number') {
            ...
            const rawBrandOrigins = (_d = (_c = existingMauticContact.fields) === null || _c === void 0 ? void 0 : _c.all) === null || _d === void 0 ? void 0 : _d.brand_origin;
            let currentBrandOrigins = [];
            if (typeof rawBrandOrigins === 'string' && rawBrandOrigins.trim() !== '') {
                ...
                currentBrandOrigins = rawBrandOrigins.filter((item) => typeof item === 'string' && item.trim() !== '');
            }
        ...
        for (const key in mauticPayloadForApi) {
            if (mauticPayloadForApi[key] !== undefined) {
                cleanPayload[key] = mauticPayloadForApi[key];
            }
        }
        ...
        const responseData = await updateMauticContact(mauticApiUrl.substring(mauticBaseUrl.length + 5), // Pass endpoint part, e.g., 'contacts/new'
        httpMethod, cleanPayload);
        console.log(`MAUTIC_SYNC: Successfully ${httpMethod === 'POST' ? 'created' : 'updated'} Mautic for ${contactInput.email}. Response contact ID:`, (_e = responseData.contact) === null || _e === void 0 ? void 0 : _e.id);
        return responseData;
    }
    catch (error) {
...
async function makeMauticApiCall(endpoint, method) {
    return await updateMauticContact(endpoint, method);
}
