...
async function generateKontextFaceLogo(prompt, modelName, aspectRatio, // Fixed for face logos
inputImageS3Url // Publicly accessible URL of the user's uploaded face
) {
    let replicatePath;
    ...
    function isNodeReadableStream(obj) { return obj instanceof Readable; }
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    function isWebReadableStream(obj) { return typeof (obj === null || obj === void 0 ? void 0 : obj.getReader) === "function"; }
    let base64Image;
    if (isNodeReadableStream(outputFromReplicate)) {
        ...
        const firstItem = outputFromReplicate[0];
        ...
        if (typeof outputFromReplicate === 'object' && outputFromReplicate !== null) {
            const errObj = outputFromReplicate;
            if (errObj.error)
                errorDetail = `AI Model Error: ${errObj.error}`;
            else if (errObj.detail)
                errorDetail = `AI Model Detail: ${errObj.detail}`;
        }
...
export const faceLogoRouter = createTRPCRouter({
    generateFaceLogo: protectedProcedure
        ...
        if ((updatedUser === null || updatedUser === void 0 ? void 0 : updatedUser.email) && env.MAUTIC_BASE_URL) {
            try {
                await syncUserToMautic({ email: updatedUser.email, name: updatedUser.name,
                    brand_specific_credits: updatedUser.gamingCredits,
                    brand_specific_plan: (_c = (_b = updatedUser.gamingPlan) === null || _b === void 0 ? void 0 : _b.toString()) !== null && _c !== void 0 ? _c : PrismaPlan.None.toString(),
        ...
        try {
            ...
            const mimeMatch = (_d = parts[0]) === null || _d === void 0 ? void 0 : _d.match(/:(.*?);/);
        ...
        if (!BUCKET_NAME)
            throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "S3_BUCKET_NAME_GAMING not configured for final upload." });
        const createdIcons = await Promise.all(b64Images.map(async (imgBase64) => {
