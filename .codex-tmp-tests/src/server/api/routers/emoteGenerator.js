...
async function uploadBase64ToS3(base64, keyPrefix) {
...
`;
const buildTextOverlayPrompt = (text, position, color) => `
...
export const emoteRouter = createTRPCRouter({
    ...
    generateBaseImage: protectedProcedure
        ...
        if (!user || ((_a = user.gamingCredits) !== null && _a !== void 0 ? _a : 0) < BASE_COST) {
            throw new TRPCError({
                code: "BAD_REQUEST",
        ...
        if (user.email && env.MAUTIC_BASE_URL) {
            await syncUserToMautic({
                email: user.email,
                name: user.name,
                brand_specific_credits: ((_c = user.gamingCredits) !== null && _c !== void 0 ? _c : 0) - BASE_COST,
                brand_specific_plan: (_e = (_d = user.gamingPlan) === null || _d === void 0 ? void 0 : _d.toString()) !== null && _e !== void 0 ? _e : PrismaPlan.None.toString(),
            }, "gaminglogoai");
        }
        return {
    ...
    generateEmotes: protectedProcedure
        ...
        if (!user || ((_a = user.gamingCredits) !== null && _a !== void 0 ? _a : 0) < totalCost) {
            throw new TRPCError({
                code: "BAD_REQUEST",
                message: "Not enough gaming credits",
            });
...
`;
            if (input.withText) {
                prompt += buildTextOverlayPrompt(emoteDef.label, input.textPosition, input.textColor);
            }
            let output;
            try {
            ...
            const base64 = await fetchAndEncodeImage(output[0]);
            const { key: s3Key, url: imageUrl } = await uploadBase64ToS3(base64, `emotes/final/${user.id}/${emoteKey.toLowerCase()}`);
            await ctx.prisma.icon.create({
                data: {
                    userId: user.id,
