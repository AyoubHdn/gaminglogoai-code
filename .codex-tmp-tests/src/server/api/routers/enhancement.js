...
async function enhanceImageWithAI(prompt, modelName, referenceS3Url) {
    const replicatePath = modelMap[modelName];
    if (!replicatePath) {
        throw new TRPCError({ code: "BAD_REQUEST", message: `Model ${modelName} not configured.` });
    }
    const replicateApiInput = modelName === "nano-banana-pro"
        ? {
            prompt,
            image_input: [referenceS3Url],
    ...
    enhanceImage: protectedProcedure
        ...
        const { count } = await ctx.prisma.user.updateMany({
            where: {
                ...
                gamingCredits: { gte: creditCost },
            },
            data: {
                gamingCredits: { decrement: creditCost },
            },
        });
        if (count <= 0) {
            throw new TRPCError({
                code: "BAD_REQUEST",
                message: "You do not have enough credits for AI enhancement.",
            });
        }
        ...
        if (count <= 0) {
            throw new TRPCError({ code: "BAD_REQUEST", message: "You do not have enough credits for AI enhancement." });
        }
        ...
        const finalS3Url = `https://${BUCKET_NAME}.s3.${env.NEXT_PUBLIC_AWS_REGION_GAMING}.amazonaws.com/${icon.id}`;
