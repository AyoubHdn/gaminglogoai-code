...
const getImagePath = (src, tool) => {
    if (!src)
        return "/images/placeholder-style.png";
...
let allPseoItems = null;
function getAllPseoItems() {
    if (allPseoItems)
        return allPseoItems;
    const items = [];
        ...
        if (category.structure === 'keys') {
            ...
            for (const itemName in dataSet) {
                if (Object.prototype.hasOwnProperty.call(dataSet, itemName)) {
                    const itemArray = dataSet[itemName];
            ...
            for (const subCategoryName in dataSet) {
                if (Object.prototype.hasOwnProperty.call(dataSet, subCategoryName)) {
                    const subCategoryItems = dataSet[subCategoryName];
 ...
 */
export function getStaticRelatedItems(options, count = 6) {
    const allItems = getAllPseoItems();
    const currentIndex = allItems.findIndex(item => item.tool === options.tool && item.name.toLowerCase() === options.excludeName.toLowerCase());
    if (currentIndex === -1) {
 ...
 */
export function getShowcaseItems(sourceData, parentKey, excludeName, count = 3) {
    const showcaseItems = [];
