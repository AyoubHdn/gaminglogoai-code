...
export function Footer() {
    ...
    const mainLinks = [
        { name: "Browse All Styles", url: "/logo-styles" },
    ...
    return (<footer className="bg-slate-100 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400">
      <div className="container mx-auto px-4 sm:px-6 py-12">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8 mb-8">
          {/* About/Brand Column */}
          <div className="col-span-2 md:col-span-4 lg:col-span-1">
            <Link href="/" className="flex items-center gap-2 mb-3 group w-fit">
              <Image src="/logo.webp" // ** REPLACE with your GamingLogoAI logo **
     alt={`${siteName} Logo`} width={32} height={32} className="rounded-md group-hover:scale-110 transition-transform" unoptimized={true} // Keep if you want to avoid Vercel optimization for this small logo
    />
            ...
            <div className="flex items-center gap-4 mt-4 hidden sm:flex"> {/* Hide on xs screens if you only want them in the first column on desktop */}
              {socialLinks.map(social => (<a key={social.name} href={social.href} target="_blank" rel="noopener noreferrer" aria-label={`Follow us on ${social.name}`} className="text-slate-500 hover:text-purple-600 dark:text-slate-400 dark:hover:text-cyan-400 transition-colors">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {generators.map(gen => (<li key={gen.name}>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {popularNiches.map(niche => (<li key={niche.name}>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {mainLinks.map(link => (<li key={link.name}><PrimaryLink href={link.url} className="text-xs hover:underline">{link.name}</PrimaryLink></li>))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {companyLinks.map(link => (<li key={link.name}><PrimaryLink href={link.url} className="text-xs hover:underline">{link.name}</PrimaryLink></li>))}
            </ul>
          </div>
        </div>
