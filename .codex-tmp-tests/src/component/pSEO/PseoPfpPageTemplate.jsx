...
const PseoPfpPageTemplate = ({ pageTitle, metaDescription, keywords, canonicalUrl, h1, heroBeforeImageSrc, heroAfterImageSrc, introParagraph, ctaText, handleCtaClick, showcaseTitle, imageShowcaseGrid, howItWorksTitle, faqTitle, faqItems, finalCtaTitle, finalCtaParagraph, relatedItems }) => {
    return (<>
      <Head>
        ...
        {faqItems.length > 0 && (<script type="application/ld+json" dangerouslySetInnerHTML={{
      ...
      <main className="bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-200 overflow-x-hidden">
        ...
        <section className="py-16 md:py-20 bg-white dark:bg-slate-900">
          <div className="container mx-auto px-4 sm:px-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {imageShowcaseGrid.map((example, index) => (<div key={index} className="group relative rounded-lg shadow-lg overflow-hidden aspect-square">
        ...
        {relatedItems.length > 0 && (<section className="py-16 md:py-20 bg-white dark:bg-slate-900">
            <div className="container mx-auto px-4 sm:px-6">
              <h2 className="text-3xl sm:text-4xl font-bold text-center mb-12 text-slate-900 dark:text-white">
                Explore Other <span className="text-purple-600 dark:text-cyan-400">PFP Styles</span>
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {relatedItems.map((item) => (<Link key={item.slug} href={`/${item.tool}/${item.categoryPath}/${item.slug}`} className="block p-4 bg-slate-50 dark:bg-slate-800 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 flex items-center group">
                    <Image src={item.exampleImage} alt={`${item.name} gaming PFP — AI generated avatar example`} width={64} height={64} className="rounded-lg mr-4 object-cover" unoptimized={true}/>
        ...
        <section className="py-16 md:py-20 bg-slate-50 dark:bg-slate-950">
          <div className="container mx-auto max-w-3xl px-4 sm:px-6">
            ...
            <div className="space-y-6">
              {faqItems.map((faq) => (<div key={faq.q} className="p-4 bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
