...
export function Step3Result() {
    var _a, _b, _c, _d;
    const { selectedPlatform, selectedTemplateId, logoSource, logoUrl, channelName, tagline, socialHandles, originalImageUrl, originalIconId, currentImageUrl, currentIconId, sessionCreditsSpent, refinementHistory, isGenerating, setCurrentStep, setOriginalImageUrl, setOriginalIconId, setCurrentImageUrl, setCurrentIconId, setSessionCreditsSpent, setRefinementHistory, setIsGenerating, startOver, clearPersistedState, } = useFunnel();
    ...
    const bannerCanvas = selectedPlatform
        ? (_a = PLATFORMS[selectedPlatform].surfaces.banner) === null || _a === void 0 ? void 0 : _a.canvas
        : null;
    const bannerWidth = (_b = bannerCanvas === null || bannerCanvas === void 0 ? void 0 : bannerCanvas.width) !== null && _b !== void 0 ? _b : 0;
    const bannerHeight = (_c = bannerCanvas === null || bannerCanvas === void 0 ? void 0 : bannerCanvas.height) !== null && _c !== void 0 ? _c : 0;
    const selectedTemplateKey = (_d = selectedTemplate === null || selectedTemplate === void 0 ? void 0 : selectedTemplate.id) !== null && _d !== void 0 ? _d : "";
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            const safeName = (channelName.trim() || "twitch-banner").replace(/[^a-z0-9_\-]/gi, "_");
            link.href = blobUrl;
    ...
    if (!selectedPlatformKey || !selectedTemplate || !bannerWidth || !bannerHeight) {
        ...
        <Button className="mt-5" onClick={() => setCurrentStep(selectedPlatformKey ? "step1" : "step0")}>
          {selectedPlatformKey ? "Back to templates" : "Back to platform selection"}
        </Button>
      </section>);
    }
    const isRefineDisabled = !currentImageUrl ||
        !currentIconId ||
        refinementPrompt.trim().length === 0 ||
        isGenerating ||
        refineBanner.isLoading;
    return (<section className="space-y-6">
      {errorToast ? (<div className="fixed right-4 top-4 z-[90] max-w-sm rounded-xl border border-red-300 bg-red-50 px-4 py-3 text-sm text-red-800 shadow-lg dark:border-red-700 dark:bg-red-950/80 dark:text-red-100">
          {errorToast}
        </div>) : null}
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr),320px]">
        <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950" style={{ aspectRatio: `${bannerWidth} / ${bannerHeight}` }}>
            {currentImageUrl ? (<img src={currentImageUrl} alt={`Generated banner preview for ${channelName || "your channel"}`} className="h-full w-full object-cover"/>) : (<div className="flex h-full items-center justify-center px-6 text-center text-sm text-slate-500 dark:text-slate-400">
                Your banner preview will appear here once generation succeeds.
              </div>)}

            {isGenerating ? (<div className="absolute inset-0 flex items-center justify-center bg-slate-950/55 backdrop-blur-sm">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Button onClick={() => setShowRegenerateConfirm(true)} disabled={isGenerating || generateBanner.isLoading}>
              Regenerate
            </Button>
            <Button variant="secondary" onClick={() => void handleDownload()} disabled={!currentImageUrl || isGenerating}>
              Download PNG
            </Button>
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/70">
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row">
              <input value={refinementPrompt} onChange={(event) => {
            if (event.target.value.length <= REFINEMENT_PROMPT_LIMIT) {
                setRefinementPrompt(event.target.value);
            }
        }} placeholder="e.g. change the background to dark blue, make the text larger, add a glowing effect" className="min-h-[48px] flex-1 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:focus:border-cyan-400 dark:focus:ring-cyan-400/20"/>
              <Button onClick={() => void runRefinement()} disabled={isRefineDisabled} isLoading={refineBanner.isLoading}>
                Refine
              </Button>
            </div>
            ...
            <div className="mt-2 flex flex-wrap items-center justify-between gap-3">
              ...
              <button type="button" onClick={handleResetToOriginal} disabled={!originalImageUrl ||
            !originalIconId ||
            currentImageUrl === originalImageUrl ||
            isGenerating} className="text-sm font-medium text-purple-600 hover:text-purple-700 disabled:cursor-not-allowed disabled:text-slate-400 dark:text-cyan-400 dark:hover:text-cyan-300 dark:disabled:text-slate-600">
                Reset to original
              </button>
            </div>
          </div>
        ...
        <aside className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <dl className="mt-4 space-y-4 text-sm">
            ...
            : logoSource === "designs"
              ...
              {logoSource !== "none" && logoUrl && (<img src={logoUrl} alt="Selected logo source" className="mt-3 h-16 w-16 rounded-lg object-cover"/>)}
            </div>
            <div>
              <dt className="font-medium text-slate-500 dark:text-slate-400">
                Channel name
            ...
            <div>
              ...
              <dd className="mt-1 text-slate-900 dark:text-white">
                {socialHandles || "No social handles"}
              </dd>
            </div>
            <div>
              <dt className="font-medium text-slate-500 dark:text-slate-400">
      ...
      {showRegenerateConfirm && (<div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/70 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-900">
            <h3 className="text-xl font-semibold text-slate-900 dark:text-white">
              Regenerate this banner?
            </h3>
            <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">
              Regenerating will cost another 3 credits and create a fresh banner
              output.
            </p>
