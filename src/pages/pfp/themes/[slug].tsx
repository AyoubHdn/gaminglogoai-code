...
const PfpThemePage: NextPage<PfpThemePageServerProps> = ({ themeName, styleItem, slug, relatedItems, otherShowcaseItems }) => {
  ...
  const handleCtaClick = () => {
    void router.push(
      `/studio?tool=pfp&game=${encodeURIComponent(themeName.toLowerCase())}`,
    );
  };

  const templateProps: PseoPfpPageTemplateProps = {
    gameTitle: themeName,
    pageTitle: `${themeName} PFP Maker - AI Themed Custom Profile Pictures`,
    metaDescription: `Generate a custom ${themeName} PFP with our AI avatar maker. Turn your photo into a unique ${themeName} profile picture for fantasy, sci-fi, horror, and more.`,
    keywords: `${themeName} pfp maker, ${themeName} pfp, ${themeName} avatar maker, custom ${themeName} avatar, ${themeName} profile picture, fantasy avatar`,
    canonicalUrl: `https://gaminglogoai.com/pfp/themes/${slug}`,
    h1: <>AI <span className="text-cyan-400">{themeName}</span> PFP Maker</>,
