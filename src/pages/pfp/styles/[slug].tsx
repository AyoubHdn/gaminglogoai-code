...
const PfpStylePage: NextPage<PfpStylePageServerProps> = ({ styleName, styleItem, slug, relatedItems, otherShowcaseItems }) => {
  ...
  const templateProps: PseoPfpPageTemplateProps = {
    gameTitle: styleName,
    pageTitle: `${styleName} PFP Maker — AI Avatar & Profile Picture Generator`,
    metaDescription: `Create a custom ${styleName} PFP from your photo with our AI avatar generator. Transform your face into a unique ${styleName} profile picture for Discord, YouTube, or Twitch.`,
    keywords: `${styleName} pfp maker, ${styleName} pfp, ${styleName} avatar maker, custom ${styleName} avatar, ${styleName} profile picture, ai avatar generator`,
    canonicalUrl: `https://gaminglogoai.com/pfp/styles/${slug}`,
    h1: <>AI <span className="text-cyan-400">{styleName}</span> PFP Maker</>,
