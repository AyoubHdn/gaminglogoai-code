...
if (!BUCKET_NAME) {
  console.error(
    "CRITICAL_ERROR: S3_BUCKET_NAME_GAMING env var not set for faceLogo.ts!",
  );
...
async function fetchAndEncodeOutputImage(url: string): Promise<string> {
  ...
  if (!response.ok) {
    const errorText = await response
      .text()
      .catch(() => `Failed to retrieve error text from ${url}`);
    console.error(
      `FACELOGO.TS: Failed to fetch image from ${url}. Status: ${response.status}. Body: ${errorText}`,
    );
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
      message: `AI model output URL error (${response.status}): ${errorText.substring(0, 100)}`,
    });
  }
...
async function generateKontextFaceLogo(
  ...
  console.log(
    `FACELOGO.TS: Calling ${modelName} with input:`,
    JSON.stringify(replicateApiInput),
  );
  ...
  const outputFromReplicate: unknown = await replicate.run(replicatePath, {
    input: replicateApiInput,
  });
  console.log(
    "FACELOGO.TS: Replicate raw output (typeof):",
    typeof outputFromReplicate,
  );
  console.log(
    "FACELOGO.TS: Replicate raw output (value):",
    JSON.stringify(outputFromReplicate).substring(0, 500) + "...",
  );
  ...
  if (isNodeReadableStream(outputFromReplicate)) {
    console.log(
      "FACELOGO.TS: Processing Node.js ReadableStream from Kontext model.",
    );
    ...
    console.log(
      "FACELOGO.TS: Processing Web ReadableStream from Kontext model.",
    );
    ...
    typeof outputFromReplicate === "string" &&
    outputFromReplicate.startsWith("http")
  ) {
    console.log(
      "FACELOGO.TS: Processing URL output from Kontext model:",
      outputFromReplicate,
    );
    base64Image = await fetchAndEncodeOutputImage(outputFromReplicate);
  } else if (
    typeof outputFromReplicate === "object" &&
    outputFromReplicate !== null &&
    "url" in outputFromReplicate &&
    typeof outputFromReplicate.url === "string" &&
    outputFromReplicate.url.startsWith("http")
  ) {
    console.log(
      "FACELOGO.TS: Processing object with URL from Kontext model:",
      outputFromReplicate.url,
    );
    base64Image = await fetchAndEncodeOutputImage(outputFromReplicate.url);
  } else if (
    Array.isArray(outputFromReplicate) &&
    outputFromReplicate.length > 0
  ) {
    // If it's an array, take the first item and process it
    console.log(
      "FACELOGO.TS: Kontext model returned an array, processing first item.",
    );
    const firstItem: unknown = outputFromReplicate[0];
    // Re-process the first item using the same logic
    if (typeof firstItem === "string" && firstItem.startsWith("http"))
      base64Image = await fetchAndEncodeOutputImage(firstItem);
    // Add other checks for stream/buffer if firstItem could be those
    else {
      console.error(
        "FACELOGO.TS: First item in Kontext output array not a URL:",
        firstItem,
      );
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "Unexpected array output format from Kontext model.",
      });
    }
  } else {
    ...
    console.error(
      "FACELOGO.TS: Unexpected output type from Kontext model:",
      outputFromReplicate,
    );
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
  ...
  if (!base64Image || base64Image.length < 100) {
    ...
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
      message: "Failed to obtain valid image data from AI model.",
    });
  }
...
export const faceLogoRouter = createTRPCRouter({
  generateFaceLogo: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      console.log(
        "FACELOGO_ROUTER: [1] generateFaceLogo mutation ENTERED. User:",
        ctx.session.user.id,
        "Model:",
        input.model,
      );
