...
function getGradientStartClass(color: string | undefined): string {
  switch (color) {
    case "blue":
      return "from-sky-500";
    case "purple":
...
function getGradientEndClass(color: string | undefined): string {
  switch (color) {
    case "blue":
...
function TemplateThumbnail({
  ...
  template: (typeof BANNER_TEMPLATES)[number];
}) {
    ...
    return (
      <div
        className={clsx(
          ...
          getGradientStartClass(firstColor),
          getGradientEndClass(secondColor ?? firstColor)
        )}
      >
        <div className="flex flex-wrap gap-2">
          {template.categories.games.slice(0, 1).map((game) => (
            <span
              ...
              className="rounded-full bg-white/20 px-2.5 py-1 text-xs font-semibold backdrop-blur"
            >
  ...
  return (
    <img
      ...
      alt={`${template.name} banner preview`}
...
export function Step1TemplateBrowser() {
  ...
  const platformTemplates = useMemo(() => {
    ...
    return BANNER_TEMPLATES.filter(
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Browse banner presets by game first. We can add deeper style filters
            later without making prompt quality harder to control right now.
          </p>
        </div>
        {activeFilterCount > 0 && (
          <button
        ...
        {!selectedPlatform ? (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center dark:border-slate-700 dark:bg-slate-900/60">
            ...
            <button
              type="button"
              onClick={() => setCurrentStep("step0")}
              className="mt-4 rounded-lg bg-purple-600 px-4 py-2 text-sm font-semibold text-white hover:bg-purple-700 dark:bg-cyan-500 dark:text-slate-950 dark:hover:bg-cyan-400"
            >
          ...
          <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center dark:border-slate-700 dark:bg-slate-900/60">
            ...
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
              {platformTemplates.length === 0
                ...
                : "Reset the selected game filter to see more banner presets."}
            </p>
            {platformTemplates.length > 0 && (
              <button
                type="button"
