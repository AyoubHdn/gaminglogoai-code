...
export function Step1TemplateBrowser() {
  ...
  const platformTemplates = useMemo(() => {
    ...
    return STREAM_SCREEN_TEMPLATES.filter(
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div>
          ...
          <h2 className="text-2xl font-semibold text-slate-900 dark:text-white">
            Choose a Game-Based Stream Screen Template
          </h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {selectedPlatform
              ? `${STREAM_SCREEN_PLATFORMS[selectedPlatform].displayName} presets`
              : "Stream screen presets"}{" "}
            built around game direction first, then stream-scene styling.
          </p>
        </div>
      </div>
      ...
      {!selectedPlatform ? (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center dark:border-slate-700 dark:bg-slate-900/60">
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            Start with the platform so we can match the template direction to the
            correct fullscreen stream canvas.
          </p>
        </div>
      ) : (
              ...
              >
                <div className="relative aspect-video overflow-hidden bg-slate-100 dark:bg-slate-800">
                  <img
                    ...
                    alt={`${template.name} stream screen preview`}
                  ...
                  <span className="absolute right-3 top-3 rounded-full bg-slate-950/80 px-2.5 py-1 text-xs font-semibold text-white shadow-sm">
                    {template.credits} credits per screen
                  </span>
                </div>
                <div className="space-y-2 p-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                      {formatGameLabel(template.categories.games[0] ?? "gaming")} stream
                      scene template
                    </p>
                  </div>
